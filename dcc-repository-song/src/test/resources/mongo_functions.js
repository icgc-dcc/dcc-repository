...
function check(o1,o2,fields) {
   ...
   function bad(field) { return !identical(field); }
...
function drop(blacklist) { return function(x) { return !blacklist.some(eq(x)) } }
...
function map_documents() {
   db.SONG.find().map(check_document).forEach(function(d) { db.status.insert(d) })
}
